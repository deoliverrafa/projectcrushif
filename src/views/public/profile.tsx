...
const ProfileLayout = () => {
  ...
  const [likeCount,
    setLikeCount] = React.useState(0);

  const [skip,
    setSkip] = React.useState(0);
  const [limit] = React.useState(5);
  const [hasMorePosts,
    setHasMorePosts] = React.useState(true);
  const [posts,
    setPosts] = React.useState < Post[] > ([]);
  ...
  const [followedUser,
    setFollowedUser] = React.useState < boolean > (false);
  const [NFollowing,
    setNFollowing] = React.useState(0);
    ...
    <React.Fragment>
      ...
      <Card className="select-none mt-1 w-full md:w-6/12">
        <div className="relative w-full h-40">
          ...
          src={
          viewingUser?.banner
          ? viewingUser?.banner: "https://img.freepik.com/fotos-premium/fundo-abstrato-da-lua-em-cores-esteticas-generative-ai_888418-6857.jpg?w=996"
          }
          alt="Banner"
      ...
      <CardHeader className="flex flex-row justify-between items-center mt-8">
        <div className="flex flex-col gap-1 w-full">
          ...
          <CardTitle className="my-0.5 text-foreground truncate max-w-72">
            ...
            ? viewingUser.userName: "Nome indisponível"}
          </CardTitle>
